...
import { DatumByteHexString } from './DatumByteHexString';
...
test('it should serialize hex string type to PlutusData representation', () => {
  ...
  const hexStrDatum = DatumByteHexString.serialize(hexStr);
...
test('it should deserialize PlutusData representation to hex string', () => {
  ...
  const strResult = DatumByteHexString.deserialize(hexStrDatum);

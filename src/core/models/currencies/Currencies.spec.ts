...
test('it should create valid instance of Currencies from array', () => {
  const currencies = Currencies.new([Currency.new(1n, spf)]);
...
test('it should get ada asset amount', () => {
  ...
  expect(adaAmount?.asset.splashId).toBe(ada.splashId);
});
...
test('it should get spf asset amount', () => {
  ...
  expect(spfAmount?.asset.splashId).toBe(spf.splashId);
});

test('it should get asset amount by asset info', () => {
  const currencies = Currencies.new(cbor);
  const spfAmount = currencies.get(spf);
  const adaAmount = currencies.get(ada);

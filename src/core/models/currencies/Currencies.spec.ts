import { Value } from '@dcspark/cardano-multiplatform-lib-browser';
...
test('it should create valid instance of Currencies from array', () => {
  const currencies = Currencies.new([Currency.new(1n, spfAssetInfo)]);
  expect(currencies).toBeInstanceOf(Currencies);
});

test('it should create valid instance of AssetAmountSet from cbor', () => {
  expect(Currencies.new(cbor)).toBeInstanceOf(Currencies);
});

test('it should create valid instance of Currencies from wasm value', () => {
  expect(Currencies.new(Value.from_cbor_hex(cbor))).toBeInstanceOf(Currencies);
});

test('it should get ada asset amount', () => {
  const currencies = Currencies.new(cbor);
  const adaAmount = currencies.ada;

  expect(adaAmount?.amount).toBe(45068135n);
  expect(adaAmount?.asset.splashId).toBe(adaAssetInfo.splashId);
});
...
test('it should get spf asset amount', () => {
  ...
  expect(spfAmount?.asset.splashId).toBe(spfAssetInfo.splashId);
});

test('it should get asset amount by asset info', () => {
  const currencies = Currencies.new(cbor);
  const spfAmount = currencies.get(spfAssetInfo);
  const adaAmount = currencies.get(adaAssetInfo);
...
test('it should return valid cbor', () => {
  const assetAmountSet = Currencies.new(cbor);

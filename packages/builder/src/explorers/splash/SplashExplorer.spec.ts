import { SplashExplorer } from './SplashExplorer.ts';
import { UTxO } from '../../core/models/UTxO/UTxO.ts';

const splashExplorer = SplashExplorer.new('mainnet');

test('it should be valid instance of splash explorer', () => {
  expect(splashExplorer).toBeInstanceOf(SplashExplorer);
  expect(splashExplorer.network).toBe('mainnet');
});

test('it should returns valid protocol params', async () => {
  const pParams = await splashExplorer.getProtocolParams();
  expect(pParams).toBeInstanceOf(Object);
});

test('it should returns valid network context', async () => {
  const nContext = await splashExplorer.getNetworkContext();
  expect(nContext).toBeInstanceOf(Object);
});

test('it returns valid UTxO', async () => {
  const uTxO = await splashExplorer.getUTxOByRef({
...
test('it returns valid UTxOs', async () => {
  const uTxOs = await splashExplorer.getUTxOsByAddress(

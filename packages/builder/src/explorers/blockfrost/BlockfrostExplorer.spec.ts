import { BlockfrostExplorer } from './BlockfrostExplorer.ts';
import { UTxO } from '../../core/models/UTxO/UTxO.ts';

const blockfrostExplorer = BlockfrostExplorer.new(
  'mainnet',
    process.env.BLOCKFROST_API_KEY!,
);

test('it should be valid instance of blockfrost explorer', () => {
  expect(blockfrostExplorer).toBeInstanceOf(BlockfrostExplorer);
  expect(blockfrostExplorer.network).toBe('mainnet');
});

test('it should returns valid protocol params', async () => {
  const pParams = await blockfrostExplorer.getProtocolParams();
  expect(pParams).toBeInstanceOf(Object);
});

test('it should returns valid network context', async () => {
  const nContext = await blockfrostExplorer.getNetworkContext();
  expect(nContext).toBeInstanceOf(Object);
});

test('it returns valid UTxO', async () => {
  const uTxO = await blockfrostExplorer.getUTxOByRef({
...
test('it returns valid UTxOs', async () => {
  const uTxOs = await blockfrostExplorer.getUTxOsByAddress(

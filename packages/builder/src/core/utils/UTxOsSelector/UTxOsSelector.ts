...
export class UTxOsSelector {
  ...
  /**
   * Adds additional UTxOs to collection
   * @param {UTxO[]} uTxOs
   * @returns {void}
   */
  addUTxOs(uTxOs: UTxO[]): void {
    this.uTxOs = uniqBy((utxo) => utxo.refHash, this.uTxOs.concat(uTxOs));
  }

  selectByOutputReference(
    outputReference: OutputReference | OutputReferenceHash,
  ): UTxO | undefined {
    return this.uTxOs.find((uTxO) =>
      typeof outputReference === 'string'
        ? uTxO.refHash === outputReference
        : uTxO.ref.txHash === outputReference.txHash &&
          uTxO.ref.index === outputReference.index,
    );
  }
  ...
  private constructor({ transactionCandidate, uTxOs }: UTxOsSelectorConfig) {
    ...
    this.uTxOs = uniqBy((uTxO) => uTxO.refHash, uTxOs);
  }

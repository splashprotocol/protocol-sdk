...
test('sessionIdValidator should pass validation with valid session ID', async () => {
  ...
  await expect(sessionIdValidator(session.id.data, session)).resolves.toBe(true);
});

test('sessionIdValidator should fail validation with different session ID', async () => {
  const wrongSessionId = 'wrong-session-id';
  
  await expect(sessionIdValidator(wrongSessionId, session)).rejects.toThrow('INVALID SESSION ID');
});

test('sessionIdValidator should fail validation with undefined session ID', async () => {
  await expect(sessionIdValidator(undefined as any, session)).rejects.toThrow('INVALID SESSION ID');
});

test('sessionIdValidator should fail validation with empty session ID', async () => {
  await expect(sessionIdValidator('', session)).rejects.toThrow('INVALID SESSION ID');
});
...
test('sessionIdValidator should fail validation with corrupted session structure', async () => {
  ...
  await expect(sessionIdValidator('test-id', corruptedSession)).rejects.toThrow('INVALID SESSION ID');
});
...
test('sessionIdValidator should fail validation with missing iv', async () => {
  ...
  await expect(sessionIdValidator('test-session-id', corruptedSession)).rejects.toThrow();
});

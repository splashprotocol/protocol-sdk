...
export interface SignTxReqValidatorParams {
  readonly event: MessageEvent<SignTxReq>;
  readonly deviceId: string;
  readonly validOrigins: string[];
  readonly expectedSource: MessageEventSource | null;
  readonly session: Session;
}
const INVALID_SCHEMA_ERROR_MESSAGE = 'INVALID SIGN TRANSACTION REQUEST SCHEMA';
const INVALID_TYPE_ERROR_MESSAGE = 'INVALID SIGN TRANSACTION REQUEST TYPE';
export const signTxReqValidator = async ({
  event,
  deviceId,
  validOrigins,
  expectedSource,
  session,
}: SignTxReqValidatorParams): Promise<true> => {

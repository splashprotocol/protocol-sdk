...
export interface GetWalletInfoReqValidatorParams {
  readonly event: MessageEvent<GetWalletInfoReq>;
  readonly deviceId: string;
  readonly validOrigins: string[];
  readonly expectedSource: MessageEventSource | null;
  readonly session: Session;
}
const INVALID_SCHEMA_ERROR_MESSAGE = 'INVALID GET WALLET INFO REQUEST SCHEMA';
const INVALID_TYPE_ERROR_MESSAGE = 'INVALID GET WALLET INFO REQUEST TYPE';
export const getWalletInfoReqValidator = async ({
  event,
  deviceId,
  validOrigins,
  expectedSource,
  session,
}: GetWalletInfoReqValidatorParams): Promise<true> => {

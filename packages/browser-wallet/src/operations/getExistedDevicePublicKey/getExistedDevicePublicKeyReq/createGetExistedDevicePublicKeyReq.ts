import { generateNonce } from '../../../common/utils/generateNonce/generateNonce.ts';
import { generateMessageForSign } from '../../../common/utils/generateMessageForSign/generateMessageForSign.ts';
import { CommunicationKeyPair } from '../../../common/models/CommunicationKeyPair/CommunicationKeyPair.ts';
import { GetExistedDevicePublicKeyReq } from '../types/GetExistedDevicePublicKeyReq.ts';

export interface CreateGetExistedDevicePublicKeyReqParams {
  readonly requestId: string;
  readonly deviceId: string;
  readonly keyPair: CommunicationKeyPair;
  readonly sessionId: string;
}

export const createGetExistedDevicePublicKeyReq = async ({
  sessionId,
  deviceId,
  keyPair,
  requestId,
}: CreateGetExistedDevicePublicKeyReqParams): Promise<GetExistedDevicePublicKeyReq> => {
  const timestamp = Date.now();
  const nonce = generateNonce();

  return {
    timestamp,
    nonce,
    type: 'GET_EXISTED_DEVICE_PUBLIC_KEY',
    payload: undefined,
    sessionId,
    signature: await keyPair.privateKey.sign(
      generateMessageForSign(undefined, timestamp, deviceId, requestId, nonce),
    ),
    deviceId,
    requestId,
  };
};

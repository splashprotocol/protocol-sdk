...
const createMockEvent = (data: ReadyRes, origin = 'https://trusted.com'): MessageEvent<ReadyRes> => ({
  data,
  origin,
...
test('readyResValidator should pass validation with valid ReadyRes', async () => {
  ...
  await expect(readyResValidator({
    event: mockEvent,
    deviceId: 'test-device-id',
...
test('readyResValidator should pass validation with multiple valid origins', async () => {
  const validResponse = createValidReadyRes();
  const mockEvent = createMockEvent(validResponse, 'https://staging.trusted.com');

...
export interface RemoveSeedPhraseReqValidatorParams {
  readonly event: MessageEvent<RemoveSeedPhraseReq>;
...
const INVALID_SCHEMA_ERROR_MESSAGE =
  'INVALID REMOVE SEED PHRASE REQUEST SCHEMA';
const INVALID_TYPE_ERROR_MESSAGE = 'INVALID REMOVE SEED PHRASE REQUEST TYPE';
export const removeSeedPhraseReqValidator = async ({
  event,
  deviceId,
  validOrigins,
  expectedSource,
  session,
}: RemoveSeedPhraseReqValidatorParams): Promise<true> => {

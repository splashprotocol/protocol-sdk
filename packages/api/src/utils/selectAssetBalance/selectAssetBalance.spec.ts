...
import { CfmmPool } from '@splashprotocol/core';
import { AssetInfo } from '@splashprotocol/core';
import { selectAssetBalance } from './selectAssetBalance.ts';
...
test('it should filters pools assets', () => {
  const balance1 = Currencies.new([Currency.ada(100n), Currency.spf(100n)]);
  const assetBalance1 = selectAssetBalance([cfmmPool], balance1);

  expect(assetBalance1.toArray().length).toBe(2);
  expect(assetBalance1.ada.amount).toBe(100n);
  expect(assetBalance1.spf.amount).toBe(100n);

  const balance2 = Currencies.new([
    Currency.ada(100n),
    Currency.spf(100n),
    cfmmPool.lq.withAmount(100n),
  ]);
  const assetBalance2 = selectAssetBalance([cfmmPool], balance2);

  expect(assetBalance2.toArray().length).toBe(2);
  expect(assetBalance2.ada.amount).toBe(100n);
  expect(assetBalance2.spf.amount).toBe(100n);
  expect(assetBalance2.get(cfmmPool.lq.asset).amount).toBe(0n);
});

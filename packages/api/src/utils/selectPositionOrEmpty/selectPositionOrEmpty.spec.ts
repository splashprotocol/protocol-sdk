...
import { selectPositionOrEmpty } from './selectPositionOrEmpty.ts';
...
test('it should return filled or empty position', () => {
  const balance1 = Currencies.new([
    Currency.ada(100n),
    Currency.spf(100n),
    cfmmPool.lq.withAmount(100n),
  ]);
  const result1 = selectPositionOrEmpty(cfmmPool, balance1);

  expect(result1).toBeInstanceOf(Position);
  expect(result1.pool).toBe(cfmmPool);
  expect(result1.lq.amount).toBe(100n);

  const balance2 = Currencies.new([Currency.ada(100n), Currency.spf(100n)]);
  const result2 = selectPositionOrEmpty(cfmmPool, balance2);

  expect(result2).toBeInstanceOf(Position);
  expect(result2.pool).toBe(cfmmPool);
  expect(result2.lq.amount).toBe(0n);
});

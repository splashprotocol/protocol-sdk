...
import { selectPositions } from './selectPositions.ts';
...
test('it should creates positions array from pools and balance', () => {
  const balance = Currencies.new([
    Currency.ada(100n),
    Currency.spf(100n),
    cfmmPool.lq.withAmount(100n),
  ]);

  const result = selectPositions([cfmmPool], balance);

  expect(result).toBeInstanceOf(Array);
  expect(result[1]).toBe(undefined);
  expect(result[0]).toBeInstanceOf(Position);
  expect(result[0].availableLq.amount).toBe(100n);
});

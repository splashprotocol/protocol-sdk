...
import { toBigNumRepresentation } from '../../utils/math/toBigNumRepresentation/toBigNumRepresentation.ts';
import { normalizeAmount } from '../../utils/math/normalizeAmount/normalizeAmount.ts';
import { toNumberRepresentation } from '../../utils/math/toNumberRepresentation/toNumberRepresentation.ts';
...
export class Currency {
  ...
  static usd(amount: bigint): Currency {
    return new Currency(amount, AssetInfo.usd);
  }
  ...
  static ada(amount: bigint): Currency {
    return new Currency(amount, AssetInfo.ada);
  }
  ...
  static spf(amount: bigint): Currency {
    return new Currency(amount, AssetInfo.spf);
  }
   ...
   * @return {Currency}
   */
  multiply(coefficient: number): Currency {
    return this.withAmount(
      BigInt(
        math.evaluate(`${this.amount} * ${coefficient}`).floor().toFixed(),
      ),
    );
  }
  ...
  private assertCurrency(
    ...
    if (this.asset.subject !== currency.asset.subject) {
      throw new AssetInfoMismatchError(
        `can't ${operationName} currencies with different asset info. ${this.asset.assetId} and ${currency.asset.assetId}`,
      );
    }
  }
}

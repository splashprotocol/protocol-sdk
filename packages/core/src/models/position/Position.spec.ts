...
test('it should create empty instance of position', () => {
  const position = Position.empty(cfmmPool);

  expect(position).toBeInstanceOf(Position);
  expect(position.pool).toBe(cfmmPool);
  expect(position.lq.asset.assetId).toBe(cfmmPool.lq.asset.assetId);
  ...
  expect(position.empty).toBe(true);
});
...
test('it should create instance of position with amount', () => {
  ...
  expect(position.empty).toBe(false);
});
